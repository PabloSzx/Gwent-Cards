...
class TopTracker {
	...
	embedData(info) {
		...
		_.forIn(info, (value, key) => {
			if (key < 10) {
				...
				txt = txt.concat(value.wins_count.toString().paddingRight(' ', 6) + ' | ');
				txt = txt.concat(value.losses_count.toString().paddingRight(' ', 12) + ' | ');
				txt = txt.concat(value.draws_count.toString().paddingRight(' ', 11) + ' | ');
				const rate = Math.ceil(value.wins_count / (value.wins_count + value.losses_count) * 100);
				txt = txt.concat(rate + '%', '\n\n');
	...
	request(msg) {
		axios
				...
				if (data) {
					msg
						...
						.catch(
							(err) =>
								err.message
									? console.error(`Error!, ${err.message} called in TopTracker.js, line 80`)
					...
					msg
						...
						.catch(
							(err) =>
								err.message
									? console.error(`Error!, ${err.message} called in TopTracker.js, line 100`)
			...
			.catch((err) => {
				err.message
					? console.error(`Error!, ${err.message} called in TopTracker.js, line 109`)
