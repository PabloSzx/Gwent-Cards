...
import { translation, nicknames, cards_language, database, emotes, self_destruct } from '../data';
import {
	stringToPathKey,
...
export default class GwentDatabase {
	...
	trimCard(param, channel) {
		...
		if (nickname[0]) {
			...
			if (ruPossibilities[0]) {
				return [ getEquivalent(bestPossibility(ruPossibilities, input)), 'ru-RU' ];
			}
			return undefined;
		} else {
	...
	static categories(data, language) {
		...
		if (data.strength > 0) {
			fields[fields.length] = `*${translation[language].power}*: **${emotes[data.strength.toString()]
		...
		if (card) {
						...
						message.author
							...
							.then((channel) => {
								channel
									...
									.then((msg) => {
										console.log(
											`Sent card info to <${message.author.username}> direct message channel`
										);
									})
									.catch(
										(err) =>
							...
							.catch((err) => {
								err.message
									? console.error(`Error!, ${err.message} called in gwentDatabase.js, line 330`)
			...
			message
				...
				.catch(
					(err) =>
						err.message
							? console.error(`Error!, ${err.message} called in gwentDatabase.js, line 352`)
