...
import { GwentDatabase, FirebaseDatabase, TopTracker, TopProTracker, UserTracker } from './';
...
export default class Discord {
	...
	start() {
		...
		this.client
			...
			.catch((err) => {
				err.message
					? console.error(`Error!, ${err.message} called in discord.js, line 28`)
		...
		if (!message.author.bot) {
			...
			try {
				if (message.isMentioned(this.client.user)) {
					...
					this.sendMessageMainChannel(message.content.trim().slice(4).trim());
				} else {
				...
				//console.error(e);
				err.message
					? console.error(`Error!, ${err.message} called in discord.js, line 81`)
	...
	checkMessage(message, content) {
		if (content.trim().slice(0, 6).toLowerCase() === '!top10') {
			this.TopTracker.request(message);
			return;
		} else if (content.trim().slice(0, 7).toLowerCase() === '!ladder') {
			this.UserTracker.request(content.slice(7).trim(), message);
			return;
		} else if (content.trim().slice(0, 9).toLowerCase() === '!protop10') {
			this.TopProTracker.request(message);
			return;
		}

		const firstBracket = content.indexOf('[');
		const secondBracket = content.substring(firstBracket).indexOf(']') + firstBracket;
		...
		if (firstBracket !== -1 && secondBracket - firstBracket !== -1) {
			...
			const long = checkChannelPermission(message.channel.id, restrictedChannels);
		...
		this.client.guilds.every(async (value) => {
			if (value.available) {
				if (value.defaultChannel) {
					if (value.defaultChannel.permissionsFor(this.client.user).has('SEND_MESSAGES')) {
						n += 1;
						...
						value.defaultChannel
							...
							.catch(
								(err) =>
									err.message
										? console.error(`Error!, ${err.message} called in discord.js, line 172`)
				...
				if (user.dmChannel) {
					user.dmChannel
						...
						.catch(
							(err) =>
								err.message
									? console.error(`Error!, ${err.message} called in discord.js, line 190`)
					...
					user.createDM((channel) => {
						channel
							...
							.catch(
								(err) =>
									err.message
										? console.error(`Error!, ${err.message} called in discord.js, line 205`)
